  ...
  return (
    <div className="grid">
      ...
      <Card title="Total Spending Limit">
        <p className="stat-value">{formatStroops(overview.totalSpendingLimit)} XLM</p>
        <p className="stat-label">Across all delegations</p>
      </Card>

      <Card title="Average Limit">
        <p className="stat-value">{formatStroops(overview.averageSpendingLimit)} XLM</p>
        <p className="stat-label">Per delegation</p>
      </Card>

      <Card title="By Status">
        <div className="status-bars">
          {Object.entries(overview.delegationsByStatus).map(([status, count]) => {
            const percentage = overview.totalDelegations > 0
